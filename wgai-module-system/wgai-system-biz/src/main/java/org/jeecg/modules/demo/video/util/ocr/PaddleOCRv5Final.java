...
/**
 * PaddleOCR v5 最终优化版 - 完整坐标信息
 * @author wggg
 * @date 2025/11/13
 */
public class PaddleOCRv5Final {
    ...
    public PaddleOCRv5Final(String detModelPath, String recModelPath,
        ...
        System.out.println("=== PaddleOCR v5 模型加载成功 ===");
        System.out.println("检测模型: " + detModelPath);
        System.out.println("识别模型: " + recModelPath);
    ...
    public List<OCRResult> ocr(String imagePath) throws OrtException, IOException {
        BufferedImage image = ImageIO.read(new File(imagePath));
        return ocr(image);
    }

    public List<OCRResult> ocr(BufferedImage image) throws OrtException {
        List<float[][]> textBoxes = detect(image);

        List<OCRResult> results = new ArrayList<>();
        for (float[][] box : textBoxes) {
            BufferedImage croppedImage = cropImage(image, box);
            String text = recognize(croppedImage);
            float confidence = 0.95f; // 可以从模型输出计算
            results.add(new OCRResult(box, text, confidence));
        }

        return results;
    }

    public List<float[][]> detect(BufferedImage image) throws OrtException {
        ...
        BufferedImage resizedImage = resizeImage(image, resizeWidth, resizeHeight);
    ...
    public String recognize(BufferedImage image) throws OrtException {
        ...
        BufferedImage resizedImage = resizeImageHighQuality(grayImage, resizeWidth, recInputHeight);
    ...
    private BufferedImage resizeImageHighQuality(BufferedImage original, int width, int height) {
    ...
    private BufferedImage cropImage(BufferedImage image, float[][] box) {
        int minX = (int) Math.max(0, Math.min(box[0][0], Math.min(box[1][0], Math.min(box[2][0], box[3][0]))));
        int maxX = (int) Math.min(image.getWidth(), Math.max(box[0][0], Math.max(box[1][0], Math.max(box[2][0], box[3][0]))));
        int minY = (int) Math.max(0, Math.min(box[0][1], Math.min(box[1][1], Math.min(box[2][1], box[3][1]))));
        int maxY = (int) Math.min(image.getHeight(), Math.max(box[0][1], Math.max(box[1][1], Math.max(box[2][1], box[3][1]))));

        int padding = 2;
        minX = Math.max(0, minX - padding);
        maxX = Math.min(image.getWidth(), maxX + padding);
        minY = Math.max(0, minY - padding);
        maxY = Math.min(image.getHeight(), maxY + padding);
    ...
    public static class OCRResult {
        public float[][] box;        // 四个角点坐标 [[x1,y1], [x2,y2], [x3,y3], [x4,y4]]
         ...
         * 格式化输出 - Python风格
         */
        public String toPythonFormat() {
            return String.format("{'text': '%s', 'box': [[%.1f,%.1f],[%.1f,%.1f],[%.1f,%.1f],[%.1f,%.1f]], 'confidence': %.2f}",
                    text,
                    box[0][0], box[0][1],
                    box[1][0], box[1][1],
                    box[2][0], box[2][1],
    ...
    public static void main(String[] args) {
        try {
            ...
            PaddleOCRv5Final ocr = new PaddleOCRv5Final(detModelPath, recModelPath,
            ...
            System.out.println("\nPython格式输出:");
            System.out.println("----------------------------------------");
            System.out.println("rec_results = [");
            for (int i = 0; i < results.size(); i++) {
                System.out.println("  " + results.get(i).toPythonFormat() +
                        (i < results.size() - 1 ? "," : ""));
            }
            System.out.println("]");
