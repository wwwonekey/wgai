...
@Slf4j
public class VideoReadInfoV5Util implements Runnable{
    ...
    public VideoReadInfoV5Util(   TabVideoUtil tabVideoUtil,String videoUrl, RedisTemplate redisTemplate, String namesUrl, String cfgUrl, String weightUrl, WebSocket webSocket){
...
//            net.setPreferableTarget(Dnn.DNN_TARGET_CUDA);
          ...
          //  log.info("开始识别"+flag);
            MapTimeUtil mapTime= VideoSendReadCfgV5Util.matlist.poll();
       ...
       //     log.info(mapTime+"开始识别"+flag);
            ...
            if(mapTime!=null){
                ...
                for (int idx : indicesArray) {
                    ...
                    //判断是否在区域内
                    boolean isPointFlag= isPointInArea(x, y,  Double.parseDouble(tabVideoUtil.getCanvasStartx()), Double.parseDouble(tabVideoUtil.getCanvasStarty()),  Double.parseDouble(tabVideoUtil.getCanvasWidth()), Double.parseDouble(tabVideoUtil.getCanvasHeight()));
                    System.out.println("[是否在区域内]:"+isPointFlag);
                    if(isPointFlag){
                        System.out.println("[在区域内]");
                    }else{
                        System.out.println("[不在区域内]");
                        continue;
                    }
    ...
    public static void main(String[] args) {
        int px = 5;     // 要检查的点的 x 坐标
        int py = 5;     // 要检查的点的 y 坐标
        ...
        boolean result = isPointInArea(px, py, x, y, width, height);
        System.out.println("Point is in area: " + result); // 输出：Point is in area: true
    }
